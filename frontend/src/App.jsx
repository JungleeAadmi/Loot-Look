...
// ... AuthForm, Dashboard, BookmarkCard, HistoryModal, ShareModal (Same as before) ...
// (Re-pasting AuthForm, Dashboard, etc. for completeness so you can copy the whole file)
...
const AuthForm = ({ type, onAuth, onSwitch }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else { onAuth(res.data.token, { id: res.data.id, username: res.data.username }); }
    ...
    <div className="flex items-center justify-center min-h-screen p-6 relative overflow-hidden">
      <div className="glass-panel p-8 md:p-12 rounded-3xl w-full max-w-md relative z-10 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 mt-8">
          ...
          <button type="submit" disabled={loading} className="w-full p-4 rounded-xl btn-primary font-bold shadow-lg flex justify-center gap-2">{loading ? <Loader2 className="animate-spin" /> : (type === 'login' ? 'Sign In' : 'Create Account')}</button>
        </form>
        <div className="mt-8 text-center pt-6 border-t border-white/5"><button className="text-slate-400 text-sm hover:text-white" onClick={onSwitch}>{type === 'login' ? "New? Sign up" : "Have account? Login"}</button></div>
      </div>
    </div>
  );
};
...
const Dashboard = ({ user, token, onLogout, openSnip, openHistory, openSettings }) => {
  ...
  const downloadCSV = () => {
    ...
    const rows = bookmarks.map(b => [`"${b.title.replace(/"/g, '""')}"`, b.current_price || 0, b.currency || 'INR', b.url, formatDate(b.created_at), formatDate(b.last_checked)]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(","), ...rows.map(e => e.join(","))].join("\n");
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a"); link.setAttribute("href", encodedUri); link.setAttribute("download", `lootlook_export_${new Date().toISOString().slice(0,10)}.csv`); document.body.appendChild(link); link.click(); document.body.removeChild(link);
  };
      ...
      <nav className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6 glass-panel p-4 rounded-2xl md:rounded-full">
        ...
        <div className="w-full md:w-auto flex justify-end gap-2 items-center">
          <div className="relative">
            <select value={filter} onChange={(e) => setFilter(e.target.value)} className="appearance-none bg-white/5 border border-white/10 text-slate-300 text-xs font-semibold rounded-full pl-4 pr-8 py-2 hover:bg-white/10 focus:outline-none cursor-pointer">
...
const BookmarkCard = ({ data, token, refreshData, onSnip, onShare, onHistory }) => {
  ...
  const fmt = (p) => { try { return new Intl.NumberFormat('en-US', { style: 'currency', currency: data.currency || 'INR', maximumFractionDigits: 0 }).format(p); } catch (e) { return p; } };
  const handleCheck = async () => { setChecking(true); try { await axios.post(`${API_URL}/bookmarks/${data.id}/check`, {}, { headers: { Authorization: `Bearer ${token}` } }); refreshData(); } catch (err) { console.error(err); } finally { setChecking(false); } };
  const handleOCR = async () => { setScanning(true); try { const res = await axios.post(`${API_URL}/bookmarks/${data.id}/ocr`, {}, { headers: { Authorization: `Bearer ${token}` } }); if(res.data.price) { alert(`Price Found: ${res.data.price}`); refreshData(); } else { alert("No price detected in image."); } } catch (err) { alert("OCR Failed"); } finally { setScanning(false); } };
  const handleDelete = async () => { if (!confirm("Are you sure you want to delete this?")) return; try { await axios.delete(`${API_URL}/bookmarks/${data.id}`, { headers: { Authorization: `Bearer ${token}` } }); refreshData(); } catch (err) {} };
  const handleCopy = async () => { try { await navigator.clipboard.writeText(data.url); alert('Link copied!'); } catch (err) { const textArea = document.createElement("textarea"); textArea.value = data.url; document.body.appendChild(textArea); textArea.select(); document.execCommand('copy'); document.body.removeChild(textArea); alert('Link copied!'); } };
  const priceColor = data.previous_price && data.current_price < data.previous_price ? 'text-emerald-400' : data.previous_price && data.current_price > data.previous_price ? 'text-red-400' : 'text-white';
  return (
    <div className="glass-card rounded-2xl flex flex-col h-full group">
      <div className="h-64 relative overflow-hidden bg-slate-900/50 cursor-pointer" onClick={onSnip}>
        {data.image_url && !imgError ? <img src={data.image_url} alt={data.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-700" onError={() => setImgError(true)} /> : <div className="absolute inset-0 flex items-center justify-center bg-slate-800"><ShoppingBag size={32} className="text-slate-600" /></div>}
...
// ... HistoryModal and ShareModal (No Change) ...
const HistoryModal = ({ item, token, onClose }) => { const [data, setData] = useState([]); const [loading, setLoading] = useState(true); useEffect(() => { axios.get(`${API_URL}/bookmarks/${item.id}/history`, { headers: { Authorization: `Bearer ${token}` } }).then(res => { const chartData = res.data.map(d => ({ date: new Date(d.recorded_at).toLocaleDateString(), price: Number(d.price) })); setData(chartData); }).finally(() => setLoading(false)); }, []); return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in" onClick={onClose}><div className="glass-panel w-full max-w-2xl p-6 rounded-2xl relative" onClick={e => e.stopPropagation()}><button onClick={onClose} className="absolute top-4 right-4"><X size={20} className="text-slate-400 hover:text-white" /></button><h3 className="text-xl font-bold text-white mb-1">Price History</h3><p className="text-sm text-slate-400 mb-6">{item.title}</p>{loading ? <div className="h-64 flex items-center justify-center"><Loader2 className="animate-spin text-indigo-500" /></div> : ( <div className="h-64 w-full"> {data.length > 1 ? ( <ResponsiveContainer width="100%" height="100%"> <AreaChart data={data}> <defs> <linearGradient id="colorPrice" x1="0" y1="0" x2="0" y2="1"> <stop offset="5%" stopColor="#818cf8" stopOpacity={0.8}/> <stop offset="95%" stopColor="#818cf8" stopOpacity={0}/> </linearGradient> </defs> <XAxis dataKey="date" stroke="#94a3b8" fontSize={12} tickLine={false} axisLine={false} /> <YAxis stroke="#94a3b8" fontSize={12} tickLine={false} axisLine={false} tickFormatter={val => `${val}`} /> <Tooltip contentStyle={{ backgroundColor: '#1e293b', border: 'none', borderRadius: '8px', color: '#fff' }} /> <Area type="monotone" dataKey="price" stroke="#818cf8" fillOpacity={1} fill="url(#colorPrice)" /> </AreaChart> </ResponsiveContainer> ) : ( <div className="flex items-center justify-center h-full text-slate-500">Not enough history yet. Check back later!</div> ) } </div> )}</div></div>); };
const ShareModal = ({ bookmark, token, onClose, refreshData }) => { const [query, setQuery] = useState(''); const [users, setUsers] = useState([]); const [currentShares, setCurrentShares] = useState([]); const [loading, setLoading] = useState(false); const isReceiver = !!bookmark.shared_by; useEffect(() => { if (!isReceiver) { axios.get(`${API_URL}/bookmarks/${bookmark.id}/shares`, { headers: { Authorization: `Bearer ${token}` } }).then(res => setCurrentShares(res.data)).catch(() => {}); } }, []); useEffect(() => { if (query.length < 2) return setUsers([]); const timer = setTimeout(async () => { setLoading(true); try { const res = await axios.get(`${API_URL}/users/search?q=${query}`, { headers: { Authorization: `Bearer ${token}` } }); setUsers(res.data); } catch (e) {} finally { setLoading(false); } }, 500); return () => clearTimeout(timer); }, [query]); const shareWith = async (userId) => { try { await axios.post(`${API_URL}/bookmarks/${bookmark.id}/share`, { receiverId: userId }, { headers: { Authorization: `Bearer ${token}` } }); const res = await axios.get(`${API_URL}/bookmarks/${bookmark.id}/shares`, { headers: { Authorization: `Bearer ${token}` } }); setCurrentShares(res.data); setQuery(''); setUsers([]); } catch (e) { alert("Failed to share"); } }; const unshareWith = async (userId) => { try { await axios.post(`${API_URL}/bookmarks/${bookmark.id}/unshare`, { receiverId: userId }, { headers: { Authorization: `Bearer ${token}` } }); setCurrentShares(currentShares.filter(u => u.id !== userId)); } catch (e) { alert("Failed to remove"); } }; const leaveShare = async () => { if (!confirm(`Stop receiving "${bookmark.title}" from @${bookmark.shared_by}?`)) return; try { await axios.delete(`${API_URL}/bookmarks/${bookmark.id}`, { headers: { Authorization: `Bearer ${token}` } }); refreshData(); onClose(); } catch (err) { alert("Failed to leave share"); } }; return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in" onClick={onClose}><div className="glass-panel w-full max-w-md p-6 rounded-2xl" onClick={e => e.stopPropagation()}><div className="flex justify-between items-center mb-4"><h3 className="text-xl font-bold text-white">Share Settings</h3><button onClick={onClose}><X size={20} className="text-slate-400 hover:text-white" /></button></div><p className="text-sm text-slate-400 mb-4 line-clamp-1 border-b border-white/10 pb-4">{bookmark.title}</p>{isReceiver ? (<div className="text-center py-4"><p className="text-slate-300 mb-4">Shared by <span className="text-purple-400 font-bold">@{bookmark.shared_by}</span></p><button onClick={leaveShare} className="w-full py-3 bg-red-600 hover:bg-red-500 rounded-xl text-white font-bold flex items-center justify-center gap-2"><LeaveIcon size={18} /> Stop Receiving</button></div>) : (<>{currentShares.length > 0 && (<div className="mb-6"><h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Currently Shared With</h4><div className="space-y-2">{currentShares.map(u => (<div key={u.id} className="flex justify-between items-center p-2 bg-white/5 rounded-lg border border-white/5"><span className="text-indigo-300 font-medium text-sm">@{u.username}</span><button onClick={() => unshareWith(u.id)} className="p-1.5 bg-red-500/10 hover:bg-red-500 rounded text-red-400 hover:text-white transition"><UserMinus size={14} /></button></div>))}</div></div>)}<h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Add Person</h4><div className="relative mb-2"><Search className="absolute left-3 top-3 text-slate-500" size={18} /><input type="text" placeholder="Search username..." className="w-full pl-10 p-3 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:border-indigo-500" autoFocus value={query} onChange={e => setQuery(e.target.value)} /></div><div className="space-y-2 max-h-40 overflow-y-auto">{loading && <div className="text-center text-slate-500 py-2">Searching...</div>}{users.map(u => (<div key={u.id} className="flex justify-between items-center p-3 hover:bg-white/5 rounded-lg transition cursor-pointer" onClick={() => shareWith(u.id)}><span className="text-white font-medium">@{u.username}</span><span className="text-xs text-indigo-400 font-bold">Add +</span></div>))}</div></>)}</div></div>); };

// --- NEW SETTINGS MODAL (UPDATED) ---
